...
test-build-all:
	@$(foreach platform, $(PLATFORMS), \
		...
		go build -o terraform-provider-byteshield; \
		rm -f terraform-provider-byteshield; \
	)

test-build: test-build-x64 test-build-x86

test-build-x64:
	GOARCH=amd64 go build -o terraform-provider-byteshield-amd64
	rm -f terraform-provider-byteshield-amd64

test-build-x86:
	GOARCH=386 go build -o terraform-provider-byteshield-386
	rm -f terraform-provider-byteshield-386
