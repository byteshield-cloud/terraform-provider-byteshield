...
func getResourceLink(resourceName string) string {
	baseName := strings.Replace(resourceName, "byteshield_", "", 1)
...
func getDataSourceLink(dataSourceName string) string {
	baseName := strings.Replace(dataSourceName, "byteshield_", "", 1)
	if *linkFormat == "github" {
...
const (
	cloudMark      = "byteshield"
	cloudTitle     = "ByteShield"
	cloudPrefix    = cloudMark + "_"
	cloudMarkShort = "en" // Keep "en" for file naming convention (files use _en_ prefix)
...
`
	...
	// Template for sidebar navigation (byteshield.erb)
	idxTPL = `
            ...
            <ul class="nav docs-sidenav">
                ...
                <li>
                    ...
                    <ul class="nav">{{if eq .Name "Provider Data Sources"}}{{range $Resource := .DataSources}}
                        ...
                        {{range $Module := .ModuleGroups}}
                        <li>
                            <a href="#">{{$Module.ModuleName}}</a>
                            <ul class="nav nav-auto-expand">
                                {{ if $Module.DataSources }}<li>
                                    <a href="#">Data Sources</a>
                                    <ul class="nav nav-auto-expand">{{range $Resource := $Module.DataSources}}
                                        <li>
                                            <a href="/docs/providers/{{$.cloud_mark}}/d/{{replace $Resource $.cloudPrefix ""}}.html">{{$Resource}}</a>
                                        </li>{{end}}
                                    </ul>
                                </li>{{ end }}
                                {{ if $Module.Resources }}<li>
                                    <a href="#">Resources</a>
                                    <ul class="nav nav-auto-expand">{{range $Resource := $Module.Resources}}
                                        <li>
...
func main() {
	...
	provider := byteshield.Provider()
	vProvider := runtime.FuncForPC(reflect.ValueOf(byteshield.Provider).Pointer())
...
func genIdx(filePath string) (prods []Product) {
	...
	// Group resources by functional modules
	for i := range prods {
		if prods[i].Name != "Provider Data Sources" {
			prods[i].ModuleGroups = groupByModule(prods[i].Resources, prods[i].DataSources)
		}
	}
...
func genMainPage(filePath string, products []Product) {
	...
	resourcesSection.WriteString("The ByteShield provider supports the following resource types:\n\n")
	...
	for _, product := range products {
		...
		// Group by modules if available
		if len(product.ModuleGroups) > 0 {
			for _, module := range product.ModuleGroups {
				resourcesSection.WriteString(fmt.Sprintf("#### %s\n\n", module.ModuleName))
				...
				if len(module.DataSources) > 0 {
					resourcesSection.WriteString("##### Data Sources\n\n")
					for _, dataSource := range module.DataSources {
			...
			// Fallback to old format if no modules
...
func getModuleName(name string) string {
	...
	for _, p := range patterns {
		if strings.HasPrefix(name, p.pattern) {
			return p.module
		}
	}
...
// groupByModule groups resources and data sources by functional module
func groupByModule(resources, dataSources []string) []ModuleGroup {
	moduleMap := make(map[string]*ModuleGroup)

	// Process resources
	for _, res := range resources {
		moduleName := getModuleName(res)
		if moduleMap[moduleName] == nil {
			moduleMap[moduleName] = &ModuleGroup{
				ModuleName:  moduleName,
				Resources:   []string{},
				DataSources: []string{},
			}
		}
		moduleMap[moduleName].Resources = append(moduleMap[moduleName].Resources, res)
	}

	// Process data sources
	for _, ds := range dataSources {
		moduleName := getModuleName(ds)
		if moduleMap[moduleName] == nil {
			moduleMap[moduleName] = &ModuleGroup{
				ModuleName:  moduleName,
				Resources:   []string{},
		...
		moduleMap[moduleName].DataSources = append(moduleMap[moduleName].DataSources, ds)
	}

	// Convert map to slice and sort by module name
	modules := make([]ModuleGroup, 0, len(moduleMap))
	for _, module := range moduleMap {
		// Sort resources and data sources within each module
		sort.Strings(module.Resources)
		sort.Strings(module.DataSources)
...
func getResourceDesc(resourceName string) string {
	descriptions := map[string]string{
		...
		"byteshield_scdn_cert_binding":                              "SCDN certificate bindings",
		"byteshield_scdn_domain_base_settings":                      "SCDN domain base settings",
		"byteshield_scdn_domain_status":                             "SCDN domain status management",
		"byteshield_scdn_domain_node_switch":                        "SCDN domain node switching",
		"byteshield_scdn_domain_access_mode":                        "SCDN domain access mode",
		"byteshield_scdn_certificate":                               "SCDN certificates",
		"byteshield_scdn_certificate_apply":                         "SCDN certificate application",
		"byteshield_scdn_rule_template":                             "SCDN rule templates",
		"byteshield_scdn_rule_template_domain_bind":                 "SCDN rule template domain bindings",
		"byteshield_scdn_rule_template_domain_unbind":               "SCDN rule template domain unbindings",
		"byteshield_scdn_network_speed_config":                      "SCDN network speed configuration",
		"byteshield_scdn_network_speed_rule":                        "SCDN network speed rules",
		"byteshield_scdn_network_speed_rules_sort":                  "SCDN network speed rules sorting",
		"byteshield_scdn_cache_rule":                                "SCDN cache rules",
		"byteshield_scdn_cache_rule_status":                         "SCDN cache rule status",
		"byteshield_scdn_cache_rules_sort":                          "SCDN cache rules sorting",
		"byteshield_scdn_security_protection_ddos_config":           "SCDN DDoS protection configuration",
		"byteshield_scdn_security_protection_waf_config":            "SCDN WAF protection configuration",
		"byteshield_scdn_security_protection_template":              "SCDN security protection templates",
		"byteshield_scdn_security_protection_template_domain_bind":  "SCDN security protection template domain bindings",
		"byteshield_scdn_security_protection_template_batch_config": "SCDN security protection template batch configuration",
		"byteshield_scdn_origin_group":                              "SCDN origin groups",
		"byteshield_scdn_origin_group_domain_bind":                  "SCDN origin group domain bindings",
		"byteshield_scdn_origin_group_domain_copy":                  "SCDN origin group domain copying",
		"byteshield_scdn_cache_clean_task":                          "SCDN cache clean tasks",
		"byteshield_scdn_cache_preheat_task":                        "SCDN cache preheat tasks",
		"byteshield_scdn_log_download_task":                         "SCDN log download tasks",
		"byteshield_scdn_log_download_template":                     "SCDN log download templates",
		"byteshield_scdn_log_download_template_status":              "SCDN log download template status",
	}
	...
	name := strings.TrimPrefix(resourceName, "byteshield_")
...
func getDataSourceDesc(dataSourceName string) string {
	descriptions := map[string]string{
		...
		"byteshield_scdn_origin":                                       "SCDN origin details",
		"byteshield_scdn_origins":                                      "SCDN origins",
		"byteshield_scdn_domain_base_settings":                         "SCDN domain base settings",
		"byteshield_scdn_access_progress":                              "SCDN access progress options",
		"byteshield_scdn_domain_templates":                             "SCDN domain templates",
		"byteshield_scdn_brief_domains":                                "SCDN brief domain information",
		"byteshield_scdn_certificate":                                  "SCDN certificate details",
		"byteshield_scdn_certificates":                                 "SCDN certificates",
		"byteshield_scdn_certificates_by_domains":                      "SCDN certificates by domains",
		"byteshield_scdn_certificate_export":                           "SCDN certificate export",
		"byteshield_scdn_rule_template":                                "SCDN rule template details",
		"byteshield_scdn_rule_templates":                               "SCDN rule templates",
		"byteshield_scdn_rule_template_domains":                        "SCDN rule template domains",
		"byteshield_scdn_network_speed_config":                         "SCDN network speed configuration",
		"byteshield_scdn_network_speed_rules":                          "SCDN network speed rules",
		"byteshield_scdn_cache_rules":                                  "SCDN cache rules",
		"byteshield_scdn_cache_global_config":                          "SCDN cache global configuration",
		"byteshield_scdn_security_protection_ddos_config":              "SCDN DDoS protection configuration",
		"byteshield_scdn_security_protection_waf_config":               "SCDN WAF protection configuration",
		"byteshield_scdn_security_protection_template":                 "SCDN security protection template details",
		"byteshield_scdn_security_protection_templates":                "SCDN security protection templates",
		"byteshield_scdn_security_protection_template_domains":         "SCDN security protection template domains",
		"byteshield_scdn_security_protection_template_unbound_domains": "SCDN security protection template unbound domains",
		"byteshield_scdn_security_protection_member_global_template":   "SCDN security protection member global template",
		"byteshield_scdn_security_protection_iota":                     "SCDN security protection IOTA",
		"byteshield_scdn_origin_group":                                 "SCDN origin group details",
		"byteshield_scdn_origin_groups":                                "SCDN origin groups",
		"byteshield_scdn_origin_groups_all":                            "SCDN all origin groups",
		"byteshield_scdn_cache_clean_config":                           "SCDN cache clean configuration",
		"byteshield_scdn_cache_clean_tasks":                            "SCDN cache clean tasks",
		"byteshield_scdn_cache_clean_task_detail":                      "SCDN cache clean task details",
		"byteshield_scdn_cache_preheat_tasks":                          "SCDN cache preheat tasks",
		"byteshield_scdn_log_download_tasks":                           "SCDN log download tasks",
		"byteshield_scdn_log_download_templates":                       "SCDN log download templates",
		"byteshield_scdn_log_download_fields":                          "SCDN log download fields",
	}
	...
	name := strings.TrimPrefix(dataSourceName, "byteshield_")
	name = strings.Replace(name, "_", " ", -1)
	return name
}
