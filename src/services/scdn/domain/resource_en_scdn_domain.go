...
import (
	...
	"github.com/byteshield-cloud/terraform-provider-byteshield/src/services/scdn/domain_group"

	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/schema"
)

// ResourceByteShieldScdnDomain returns the SCDN domain resource
func ResourceByteShieldScdnDomain() *schema.Resource {
	return &schema.Resource{
		...
		Importer: &schema.ResourceImporter{
			StateContext: resourceScdnDomainImport,
		},
...
func resourceScdnDomainCreate(d *schema.ResourceData, m interface{}) error {
	client := m.(*connectivity.ByteShieldClient)
...
// resourceScdnDomainImport handles importing SCDN domain resources
// Supports importing by domain ID (numeric) or domain name
func resourceScdnDomainImport(ctx context.Context, d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {
	client, ok := m.(*connectivity.ByteShieldClient)
	if !ok {
		return nil, fmt.Errorf("invalid client type: %T", m)
	}
	...
	domainID, err := strconv.Atoi(importID)
	var domainInfo *scdn.DomainInfo

	if err == nil {
		// Import by numeric ID
		log.Printf("[DEBUG] Importing domain by ID: %d", domainID)
		req := scdn.DomainListRequest{
			ID:       domainID,
			Page:     1,
			PageSize: 1,
		...
		response, err := service.ListDomains(req)
		if err != nil {
			return nil, fmt.Errorf("failed to query domain by ID %d: %w", domainID, err)
		}
		if len(response.Data.List) > 0 {
			for _, domain := range response.Data.List {
		...
		log.Printf("[DEBUG] Importing domain by name: %s", importID)
		req := scdn.DomainListRequest{
			Domain:   importID,
			Page:     1,
			PageSize: 100,
		...
		response, err := service.ListDomains(req)
		if err != nil {
			return nil, fmt.Errorf("failed to query domain by name %s: %w", importID, err)
		}
		if len(response.Data.List) > 0 {
			for _, domain := range response.Data.List {
				if domain.Domain == importID {
					domainInfo = &domain
					break
				}
			}
	...
	if domainInfo == nil {
		return nil, fmt.Errorf("domain not found: %s", importID)
	}

	// Set the resource ID to the numeric ID
	d.SetId(strconv.Itoa(domainInfo.ID))

	// Set domain name for Read function
	if err := d.Set("domain", domainInfo.Domain); err != nil {
		return nil, fmt.Errorf("failed to set domain: %w", err)
	}

	// Call Read to populate all fields
	if err := resourceScdnDomainRead(d, m); err != nil {
		return nil, fmt.Errorf("failed to read domain after import: %w", err)
	}
