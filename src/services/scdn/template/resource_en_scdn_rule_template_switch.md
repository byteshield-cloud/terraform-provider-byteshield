# byteshield_scdn_rule_template_switch
...
```hcl
resource "byteshield_scdn_rule_template_switch" "example" {
...
* `new_tpl_id` - (Required) New template ID to switch to. When `new_tpl_type` is `global` or `only_domain`, pass 0.
* `new_tpl_type` - (Required) New template type. Valid values: `only_domain`, `more_domain`, `global`.
...
```
$ terraform import byteshield_scdn_rule_template_switch.example 12345:1001,1002
```
