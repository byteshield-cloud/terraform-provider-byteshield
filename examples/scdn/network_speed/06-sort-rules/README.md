...
1. **Have existing rules** in the specified `config_group`
   - You can query existing rules using the `byteshield_scdn_network_speed_rules` data source
   - Example:
     ```hcl
     data "byteshield_scdn_network_speed_rules" "list" {
       business_id   = 1246
       business_type = "tpl"
       config_group  = "customized_req_headers_rule"
     }
...
1. **Query existing rules**:
   ...
   output "current_rule_ids" {
     value = [for rule in data.byteshield_scdn_network_speed_rules.example.list : rule.id]
   }
   ```
