...
This example demonstrates how to import existing ByteShield SCDN resources into Terraform state using the `terraform import` command.

## Prerequisites

1. Valid ByteShield API credentials (access_key and secret_key)
2. Existing resources in ByteShield SCDN that you want to import
3. Resource IDs from the ByteShield console or API
...
```bash
terraform import byteshield_scdn_domain.example <domain_id>
```

**Example:**
```bash
terraform import byteshield_scdn_domain.example 102008
```

### 2. Origin Group Resource

```bash
terraform import byteshield_scdn_origin_group.example <origin_group_id>
```

**Example:**
```bash
terraform import byteshield_scdn_origin_group.example 85
```

### 3. Certificate Resource

```bash
terraform import byteshield_scdn_certificate.example <certificate_id>
```

**Example:**
```bash
terraform import byteshield_scdn_certificate.example 456
```

### 4. Certificate Binding Resource

```bash
terraform import byteshield_scdn_cert_binding.example "<domain_id>:<certificate_id>"
```

**Example:**
```bash
terraform import byteshield_scdn_cert_binding.example "123:456"
```

### 5. Origin Resource

```bash
terraform import byteshield_scdn_origin.example <origin_id>
```
...
```bash
# Import domain
terraform import byteshield_scdn_domain.example $(terraform output -raw domain_id_to_import)
...
terraform state show byteshield_scdn_domain.example
```
...
1. **Get the domain ID** from ByteShield console or API
...
- Check if the resource exists in ByteShield console
