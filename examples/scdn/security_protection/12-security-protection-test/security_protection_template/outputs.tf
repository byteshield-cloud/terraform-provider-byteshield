output "id" {
  ...
  value       = byteshield_scdn_security_protection_template.this.id
}

output "business_id" {
  description = "Security protection template business ID."
  value       = byteshield_scdn_security_protection_template.this.business_id
}

output "name" {
  description = "Security protection template name."
  value       = byteshield_scdn_security_protection_template.this.name
}

output "type" {
  description = "Security protection template type."
  value       = byteshield_scdn_security_protection_template.this.type
}

output "created_at" {
  description = "Security protection template creation time."
  value       = byteshield_scdn_security_protection_template.this.created_at
}

output "bind_domain_count" {
  description = "Number of domains bound to the security protection template."
  value       = byteshield_scdn_security_protection_template.this.bind_domain_count
}

output "fail_domains" {
  description = "Domains that failed during template creation."
  value       = byteshield_scdn_security_protection_template.this.fail_domains
}

output "batch_config_id" {
  description = "Terraform resource ID for the optional batch config."
  value       = try(byteshield_scdn_security_protection_template_batch_config.this[0].id, null)
}

output "fail_templates" {
  description = "Templates that failed during batch config."
  value       = try(byteshield_scdn_security_protection_template_batch_config.this[0].fail_templates, {})
}
