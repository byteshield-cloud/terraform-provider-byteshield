...
data "byteshield_scdn_domain_groups" "all" {
  page     = 1
  per_page = 20
}

# Query domain groups by name
data "byteshield_scdn_domain_groups" "by_name" {
...
data "byteshield_scdn_domain_groups" "by_domain" {
...
output "all_groups" {
  value       = data.byteshield_scdn_domain_groups.all.list
  description = "All domain groups"
}

output "total_groups" {
  value       = data.byteshield_scdn_domain_groups.all.total
  description = "Total number of domain groups"
}

output "filtered_by_name" {
  value       = var.group_name != "" ? data.byteshield_scdn_domain_groups.by_name[0].list : []
  description = "Groups filtered by name"
}

output "filtered_by_domain" {
  value       = var.domain != "" ? data.byteshield_scdn_domain_groups.by_domain[0].list : []
