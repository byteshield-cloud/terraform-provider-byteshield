...
#
#   terraform import byteshield_scdn_domain_group.existing <group_id>
#
# Example:
#   terraform import byteshield_scdn_domain_group.existing 111
# ============================================================

# This resource manages an EXISTING domain group
# You MUST import it first before applying
resource "byteshield_scdn_domain_group" "existing" {
...
data "byteshield_scdn_domain_group_domains" "group_domains" {
  group_id   = tonumber(var.group_id)
  depends_on = [byteshield_scdn_domain_group.existing]
}

output "group_id" {
  value       = byteshield_scdn_domain_group.existing.id
  description = "The domain group ID"
}

output "group_name" {
  value       = byteshield_scdn_domain_group.existing.group_name
  description = "The domain group name"
}

output "bound_domains" {
  value       = data.byteshield_scdn_domain_group_domains.group_domains.list
  description = "List of domains bound to the group"
}

output "total_domains" {
  value       = data.byteshield_scdn_domain_group_domains.group_domains.total
