...
3. An existing SSL certificate ID (from ByteShield SSL certificate management)
...
2. Update `terraform.tfvars` with:
   ...
   - `ca_id` - The ID of an SSL certificate (from ByteShield SSL certificate management)
...
resource "byteshield_scdn_cert_binding" "example" {
  domain_id = byteshield_scdn_domain.example.id
