# Example 2: Create Cache Rule
# This example demonstrates how to create a cache rule
...
variable "conf" {
  description = "Cache rule configuration"
  type = object({
    nocache = bool
...
output "rule_id" {
  description = "Created cache rule ID"
  value       = byteshield_scdn_cache_rule.example.id
}

output "rule_status" {
  description = "Cache rule status"
  value       = byteshield_scdn_cache_rule.example.status
}
